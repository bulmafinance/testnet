...
import "../../commons/solidity-utils/openzeppelin/token/ERC20/IERC20.sol";
...
import "../voucher-core/VoucherCore.sol";
...
contract ConvertibleVoucher is
    IConvertibleVoucher,
    VoucherCore,
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        solver = IIVOSolver(solver_);

        ERC165Upgradeable._registerInterface(
            type(IConvertibleVoucher).interfaceId
        );
    }
    ...
    function mint(
    ...
    ) external override nonReentrant returns (uint256 slot, uint256 tokenId) {
        ...
        uint256 units = convertiblePool.mintWithUnderlyingToken(
            _msgSender(),
            slot,
            tokenInAmount_
        );
    ...
    function claimTo(
        ...
        ) = convertiblePool.claim(
                voucherSlotMapping[tokenId_],
                to_,
                claimUnits_
            );
        ...
        returns (ConvertibleVoucherSnapshot memory snapshot)
    {
        ...
        snapshot.slotDetail = convertiblePool.getSlotDetail(
            voucherSlotMapping[tokenId_]
        );
    }
    ...
    function underlyingVestingVoucher()
        external
        view
        override
        returns (address)
    ...
    function setVoucherDescriptor(address newDescriptor_) external onlyAdmin {
        require(
            newDescriptor_ != address(0),
            "newDescriptor can not be 0 address"
        );
        emit SetDescriptor(address(voucherDescriptor), newDescriptor_);
        voucherDescriptor = IVNFTDescriptor(newDescriptor_);
    }

    function setSolver(IIVOSolver newSolver_) external onlyAdmin {
